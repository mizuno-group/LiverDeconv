...
class Ctrlnorm_comb2ref():
    ...
    def optim_loop(self,raw_ref=None,ann_df=None,candi=['B','Basophil','CD4','CD8','Cholangiocyte','Eosinophil','Hepatocyte','Kupffer','LSEC','Monocyte','NK','Neutrophil','Stellate'],
                   target=["Monocyte"],dec_names=[["Monocyte"]],val_names=[["Monocyte"]],do_plot=False):
        """ Calculate and obtain the correlation coefficient for all combinaitons of reference cell tyopes.
        Args:
            raw_ref (DataFrame): Cell-type gene expression profile (reference) on linear scale. Defaults to None.

                     NK_1    NK_2    NK_3,  ...
            gene1    11.2    23.5    0.98
            gene2    0.00    0.00    0.00
            gene3    3801.0  3838.3  591.0

            ann_df (DataFrame): DataFrae for gene name conversion. Defaults to None.
            Gene stable ID	MGI symbol
            ...
            ...	...	...
            56291	ENSMUSG00000082803	Gm26460
            56292	ENSMUSG00000118095	Gm50435

            combination (tuple): Reference cell type combination. Defaults to ('B', 'CD4', 'NK').
            target (list): Target cell type to be estimated. Defaults to ["Monocyte"].
            dec_names (list): Target cell name registered in deconvolution. Defaults to [["Monocyte"]].
            val_names (list): Target cell name registered in reference. Defaults to [["Monocyte"]].
            do_plot (bool): _description_. Defaults to False.
            dpi (int): DPI for display. Defaults to 100.
        """
        self.raw_ref = raw_ref
        other_cells = sorted(list(set(candi)-set(target)))
        self.all_cor = [] # store correlation value
        names = []
        for i in range(1,len(other_cells)+1):
            ...
            for factor in l:
                ...
                if ann_df is None:
                    dat.pre_processing(do_ann=False,ann_df=None,do_log2=True,do_quantile=True,do_trimming=False,do_drop=False) # log2 --> quantile
                else:
                    dat.pre_processing(do_ann=True,ann_df=ann_df,do_log2=True,do_quantile=True,do_trimming=False,do_drop=False)
                dat.narrow_intersec()
    ...
    def optimized_single(self,raw_ref=None,ann_df=None,combination=('B', 'CD4', 'NK'),target=["Monocyte"],dec_names=[["Monocyte"]],val_names=[["Monocyte"]],do_plot=False,dpi=100):
        """ 
        Args:
            raw_ref (DataFrame): Cell-type gene expression profile (reference) on linear scale. Defaults to None.
        ...
        """
        self.raw_ref = raw_ref
        # extract the target cell type combination
